...
> [!IMPORTANT]
> This project is in an early infrastructure milestone. Configuration, playlist
> parsing, logging, safe ASI initialization, tests, and build automation exist.
> Online audio playback and GTA III radio hooks do not.
...
| Online audio playback | Planned; null backend only |
...
5. `StreamManager` owns exactly one stream through a replaceable `AudioBackend`.
6. When no safe online replacement is possible, the original radio remains active.

Steps 1 and 2 have an initial implementation. Steps 3–5 remain inactive where they
depend on game hooks or a real audio backend.
...
URLs are configuration data and are not fetched in this milestone. Do not put
credentials or private tokens in a configuration file shared with bug reports.

## Limitations

- There is no real audio backend.
- There are no installed game hooks or memory patches.
- AAC, MP3, HTTPS, reconnect, and buffering have not been runtime-validated.
...
- Select and prototype a redistributable Windows x86 audio backend.
...
downloaded only for test builds under the Boost Software License 1.0. Runtime and
future dependency notes are recorded in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
